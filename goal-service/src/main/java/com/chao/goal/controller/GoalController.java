...
import com.chao.goal.service.GoalCrudService;
import com.chao.goal.service.TaskService;
import com.chao.goal.service.AiTaskOrchestrator;
import com.chao.goal.service.JournalService;
...
public class GoalController {
    ...
    public Result<GoalDto> createGoal(@RequestParam Long userId, @RequestBody String goalDescription) {
        return Result.success(aiTaskOrchestrator.createGoalAndStartAi(userId, goalDescription));
    }
    ...
    public Result<Goal> createGoalRecord(
        ...
        return Result.success(goalCrudService.createGoal(userId, title, description, DateUtils.parseLocalDateTime(deadline)));
    }

    @GetMapping
    public Result<List<Goal>> listGoals(@RequestParam Long userId) {
        return Result.success(goalCrudService.listGoals(userId));
    }

    @GetMapping("/{goalId}")
    public Result<Goal> getGoal(@PathVariable Long goalId) {
        return Result.success(goalCrudService.getGoal(goalId));
    }
    ...
    public Result<String> updateGoal(
            ...
            @RequestParam(required = false) String deadline) {
        goalCrudService.updateGoal(goalId, title, description, status, DateUtils.parseLocalDateTime(deadline));
        return Result.success("更新成功");
    }

    @DeleteMapping("/{goalId}")
    public Result<String> deleteGoal(@PathVariable Long goalId) {
        goalCrudService.deleteGoal(goalId);
    ...
    public Result<java.util.Map<String, Long>> countUnfinishedTasks(@PathVariable Long goalId) {
        return Result.success(java.util.Map.of("unfinished", goalCrudService.countUnfinishedTasks(goalId)));
    }
    ...
    public Result<GoalTask> createTask(
            ...
            @RequestParam(required = false) String deadline) {
        try {
            return Result.success(taskService.createTask(userId, goalId, parentId, title, description, priority, estimatedMinutes, DateUtils.parseLocalDateTime(deadline)));
        } catch (TaskService.DuplicateTaskException e) {
    ...
    public Result<List<GoalTask>> listTasks(@PathVariable Long goalId, @RequestParam Long userId) {
        return Result.success(taskService.listTasksByGoal(userId, goalId));
    }

    @GetMapping("/journals")
    public Result<List<UserJournal>> listJournals(@RequestParam Long userId, @RequestParam(required = false) Long goalId) {
        return Result.success(goalCrudService.listJournals(userId, goalId));
    }
    ...
    public Result<List<GoalTaskDto>> getPendingTasks(@RequestParam Long userId) {
        return Result.success(taskService.getPendingTasks(userId));
    }
    ...
    public Result<String> moveTaskToGoal(@RequestParam Long userId, @RequestParam Long taskId, @RequestParam Long goalId) {
        taskService.moveTaskToGoal(userId, taskId, goalId);
    ...
    public Result<List<GoalTaskDto>> getTasksByIds(@RequestParam List<Long> taskIds) {
        return Result.success(taskService.getTasksByIds(taskIds));
    }
    ...
    public Result<String> regenerateTasks(@PathVariable Long goalId, @RequestParam Long userId, @RequestBody(required = false) String feedback) {
        aiTaskOrchestrator.regenerateTasks(userId, goalId, feedback);
    ...
    public Result<String> createJournal(
        ...
        journalService.save(userId, goalId, content, mood);
    ...
    public Result<java.util.List<String>> getDistinctTopics() {
        java.util.List<String> topics = goalCrudService.getDistinctGoalTopics();
