...
public class TaskAdviceAiService {
    ...
    private String buildSchedulePrompt(List<ScheduleAdviceItem> items, String moodHint) {
        ...
        try {
            ...
            for (ScheduleAdviceItem it : items) {
                if (it == null || it.getTaskId() == null) continue;
                Map<String, Object> m = new LinkedHashMap<>();
                m.put("taskId", it.getTaskId());
                m.put("title", it.getTitle());
                m.put("description", it.getDescription());
                m.put("startTime", it.getStartTime());
                m.put("endTime", it.getEndTime());
                m.put("timeBudgetMinutes", it.getTimeBudgetMinutes());
                arr.add(m);
            }
    ...
    private ScheduleAdviceResponse parseScheduleAdvice(String text) {
        try {
            ...
            Map<Long, TaskAdviceDto> items = new HashMap<>();
            ...
            if (itemsNode != null && itemsNode.isObject()) {
                itemsNode.fields().forEachRemaining(e -> {
                    try {
                        ...
                        items.put(taskId, new TaskAdviceDto(start, done));
                    } catch (Exception ignored) {
                    }
                });
            }
