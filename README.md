...
补充：resource-search 内置定时爬虫（传统 HTTP 抓取 + 规则过滤 + 去重，可选 AI 审核），用于持续补全 `course_resources`：

- 开关：`smartplanner.crawler.enabled`（默认 true）
- AI 审核开关：`smartplanner.crawler.ai-review.enabled`（默认 false）
- 去重：优先 URL 归一（canonicalize）+ ES 判断是否已存在，再结合标题规则去重
- 实现入口：`ResourceService.crawlResources()`（见 [ResourceService.java](file:///c:/Users/%E5%88%98%E8%B6%85/Documents/SmartPlanner/resource-search/src/main/java/com/chao/resource/service/ResourceService.java)）
...
- listPunchRecords(taskId, from, to) — 查询打卡记录，判断任务是否真的完成
...
- searchForUser(query, topK) — 混合检索（RedisStack 向量检索 + 关键词兜底），从用户随笔 + 课程资源中检索最相关内容

**Agent 行为约束（防止编造/跑偏）**

- 涉及【我有哪些任务/排程/是否完成】等事实类问题，必须先调工具确认，严禁编造
- 输出为纯文本，不要 Markdown 符号（如 **、#、`）
