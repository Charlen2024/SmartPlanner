...
- `POST /api/schedule/import?userId=...`：导入课表文件（支持 iCalendar `.ics`、CSV、Excel `.xlsx/.xls`，自动识别中英文表头，解析 RRULE/BYDAY 循环规则）
- `GET /api/schedule/free-time?userId=...&date=YYYY-MM-DD`：计算当天空闲时段（识别课程间碎片时间，填补时间空窗）
...
### 8.9 Agent：13 个 ToolCallback 驱动的智能对话

Agent 基于 Spring AI Alibaba ReactAgent + RedisSaver 实现多轮对话，通过 13 个 @Tool 方法获取真实数据，不走硬编码短路：
